...
//ukradzione od errichto z acmlib (https://github.com/mareksom/acmlib/blob/master/code/kamil/fast_bignum.cpp)
  ...
  // sort(deg.begin(), deg.end());
	...
	for (auto x: deg){
		...
		for (auto v: post){
			...
			Big a = dp[v]/(int)(g[v].size());
			Big b = dp[v]%(int)(g[v].size());
			if (a.iszero() || !b.iszero()) {
				ok = 0;
				break;
			}
			for (auto u: g[v]) dp[u] += a;
		}
		if (!ok){
			ans*=x;
			git[x] = 0;
