...
void solve(){
	...
	for (int i = 0; i<k; i++){
		if (pq2.empty()){
			...
			//cerr << "pq2 + " << curr.lx << " " << curr.rx << "\n";
			if (lx <= curr.lx-1) {
				pq.push({smax.query(1, 0, smax.size-1, lx, curr.lx-1).val, lx, curr.lx-1});
				//cerr << "pq + " << lx << " " << curr.lx-1 << "\n";
			}
			if (curr.rx+1 <= rx) {
				pq.push({smax.query(1, 0, smax.size-1, curr.rx+1, rx).val, curr.rx+1, rx});
				//cerr << "pq + " << curr.rx+1 << " " << rx << "\n";
			}
			continue;
		}
		if (pq.empty()){
			...
			//cerr << "pq + " << curr.lx << " " << curr.rx << "\n";
			if (lxm <= curr.lx-1) {
				pq2.push({smin.query(1, 0, smin.size-1, lxm, curr.lx-1).val, lxm, curr.lx-1});
				//cerr << "pq2 + " << lxm << " " << curr.lx-1 << "\n";
			}
			if (curr.rx+1 <= rxm) {
				pq2.push({smin.query(1, 0, smin.size-1, curr.rx+1, rxm).val, curr.rx+1, rxm});
				//cerr << "pq2 + " << curr.rx+1 << " " << rxm << "\n";
			}
			continue;
		}
