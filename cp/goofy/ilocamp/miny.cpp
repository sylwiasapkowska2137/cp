...
node comb(int x){
	return node{empty, Tab[2*x].lx, Tab[2*x+1].rx };
}

void build(vi scaler){
	rep(i, 0, sz(scaler)) Tab[i+size].lx = Tab[i+size].rx = scaler[i];
	rep(i, sz(scaler), size) Tab[i+size].lx = Tab[i+size].rx = INF;
	repd(i, size-1, 1) {
		Tab[i] = comb(i);
		graf[i].pb(2*i);
		graf[i].pb(2*i+1);
