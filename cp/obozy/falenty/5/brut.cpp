...
void backtrack_down(int a, int b, int v){
	...
	if (b+1 <= m) backtrack_down(a, b+1, v^A[a][b+1]);
}

void backtrack_up(int a, int b, int v){
	if (b == n-a+1){
		int y = (int)(lower_bound(values[a][b].begin(), values[a][b].end(), (k^v^A[a][b]))-values[a][b].begin());
		int z = (int)(upper_bound(values[a][b].begin(), values[a][b].end(), (k^v^A[a][b]))-values[a][b].begin());
