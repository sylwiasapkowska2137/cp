...
void solve(){
    ...
    vector<vector<int>>jump(n+1, vector<int>(K, 0));
    vector<vector<int>>minn(n+1, vector<int>(K, 0));
    vector<int>D2(n+1), D3(n+1);
	...
	function<void(int)>solve = [&](int type){
		...
		int s = (int)big[type].size();
		for (int i = 0; i<s-1; i++)	v.emplace_back(lca(v[i], v[i+1]));
		sort(v.begin(), v.end());
		v.erase(unique(v.begin(), v.end()), v.end());
		sort(v.begin(), v.end(), [&](auto a, auto b){return pre[a] < pre[b];});
		debug(v);
		...
		for (int i = 0; i<(int)v.size()-1; i++){
			...
			int l = lca(v[i], p);
			if (l == p) continue;
			G[l].emplace_back(p, D[p]-D[l]);
			G[p].emplace_back(l, D[p]-D[l]);
			// debug(l, p, D[p]-D[l]);
		}
		queue<int>q;
		...
		for (auto &[a, b, idx]:Q[type]){
			...
			// debug(a, b, L, d);
			ans[idx] = d+min(call(a, L), call(b, L));
		}
