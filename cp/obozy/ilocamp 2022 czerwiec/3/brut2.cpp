...
void gen(vector<Point>&p, vector<int>idx, int so_far, int k){
	if ((int)idx.size() == k){
		int curr = 0;
		for (int i = 0; i<(int)idx.size(); i++){
			curr += pole(p[idx[0]], p[idx[i]], p[idx[(i == (int)idx.size()-1?0:i+1)]]);
		}
		//debug(idx, curr);
		ret = max(ret, curr);
		return;
	}
	if (so_far >= (int)p.size()) return;
	gen(p, idx, so_far+1, k);
	idx.emplace_back(so_far);
	gen(p, idx, so_far+1, k);
}
