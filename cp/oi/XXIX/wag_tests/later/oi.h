/*
   ...
   1.4: (Stanislaw Findeisen) Poprawki w klasach MultipleRecursiveGenerator i Random:
        - zidentyfikowanie kilku zalozen o wielkosciach i zakresach typow calkowitych
          na ktorych istniejacy kod zdaje sie polegac, i dopisanie stosownych asercji
          (w czasie kompilacji)
        - uproszczenie makra RAND_TYPE
        - implementacja funkcji int randSInt(int, int).
*/
...
unsigned int MultipleRecursiveGenerator::next16Bits() {
  ...
  for (int i = 0; i < n_; ++i) {
    res = (res + (unsigned long long)A_[i] * X_[i]) % modulo_;
