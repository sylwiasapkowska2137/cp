...
void centroid(int v){ //lca niepionowych sciezek
	v = get(v, dfs(v, v), v);
	// debug(v);
	for (auto [x, c]:g[v]){
		if (vis[x]) continue;
	...
	if (t[v] != oo) curr.emplace_back(t[v], v);
	...
	for (auto [tt, u]:curr){
		// debug(u, ans);
		if (d[par[u]] < d[v] && u != v){
			//u jest wierzcholkiem naddrzewa
			sum += depth[lca(u, v)];
			ans[tt] += depth[lca(u, v)]*all;
			// ans[tt] += sum;
			continue;
		}
		if (u == v){
