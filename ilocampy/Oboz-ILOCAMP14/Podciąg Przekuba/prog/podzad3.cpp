...
short int* dp[2007][4007];
short int* z[2007][4007];
...
int main()
{
	...
	for (int i=0; i<=n; i++)
	{
		for (int j=0; j<=w+2; j++)
		{
			dp[i][j]= new short int[k+3];
			z[i][j]= new short int[k+3];
			memset(dp[i][j], -1, sizeof(short int)*(k+2));
			memset(z[i][j], -1, sizeof(short int)*(k+2));
		}
	}
	for (int i=1; i<=k; i++)
		dp[0][1][i]=0;
