...
{
    k = getLL(testin, 1, 2000);
	ignoreSpaces(testin);
    getNewLine(testin);
    
    n = getLL(testin, 1, 2000);
	ignoreSpaces(testin);
    getNewLine(testin);
    for (int i=1; i<=n; i++)
    {
		tab1[i]=getLL(testin, 1, k);
		if (i<n)
			getSpace(testin);
	}
	ignoreSpaces(testin);
    ...
    m = getLL(testin, 1, 2000);
	ignoreSpaces(testin);
    getNewLine(testin);
    for (int i=1; i<=m; i++)
    {
		tab2[i]=getLL(testin, 1, k);
		if (i<m)
			getSpace(testin);
	}
	ignoreSpaces(testin);
    ...
    con_res=getLL(appout, 1, max(n, m)+1);
	ignoreSpaces(appout);
    getNewLine(appout);
    for (int i=1; i<=con_res; i++)
    {
		wyn[i]=getLL(appout, 1, k);
		if (i<con_res)
			getSpace(appout);
	}
	ignoreSpaces(appout);
    ...
    our_res=getLL(testout, 1, max(n, m)+1);
    ...
    {
		l2++;
		while(l2<=m && tab2[l2]!=wyn[i])
			l2++;
	}
	
	if (l1<=n || l2<=m)
		throw WrongAnswer("Ciąg jest podciągiem któregoś z danych ciągów");
