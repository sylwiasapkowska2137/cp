...
LL score(FILE *out)
    {
    LL expected_score = getLL(out, 0, 1e18);
    ignoreSpaces(out);
    getNewLine(out);
    FOR(i, 1, k)
        {
    ...
    FOR(i, 1, n)
        ...
        if(cnt[i] > 1)
            {
            throw WrongAnswer("Wierzchołki na wyjściu się powtarzają");
            }
        }
    ...
    FOR(i, 1, n-1)
        {
        score += (cnt[a[i]] || cnt[b[i]]) * w[i];  
        }
    
    cerr<<score<<" "<<expected_score<<endl;
    
    if(score != expected_score)
        {
        throw WrongAnswer("Wypisany wynik nie jest równy rzeczywistemu wynikowi");
        }
...
void spr(FILE *testin, FILE *appout, FILE *testout, Judge &judge)
{
    n = getLL(testin, 30000, 100000);
    getSpace(testin);
	k = getLL(testin, 1, n);
	ignoreSpaces(testin);
    getNewLine(testin);
    ...
    for (int i=1; i<n; i++)
    {
        a[i] = getLL(testin, 1, n);
        getSpace(testin);
        b[i] = getLL(testin, 1, n);
        getSpace(testin);
        w[i] = getLL(testin, 1, 1000000);
        ignoreSpaces(testin);
		if (i == n-1)
    ...
    LL myscore = score(testout);
    LL theirscore = score(appout);
    
    
    LD result = (LD)theirscore / myscore * 10;
    
    char buf[30];
    sprintf(buf, "%.5Lf", result);
    
    judge.writeResult(buf);
}
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        spr(judge.TESTIN, judge.APPOUT, judge.TESTOUT, judge);
    }
