...
void spr(FILE *testin, FILE *appout, FILE *testout) {
    ...
    if(ss1.size() > 100000 || ss1.size() != ss2.size() || ss1 >= ss2) throw WrongAnswer("Błędny input!");


    fscanf(testout, "%s", ans);
    ...
    for (int i = 0; i < sans.size(); i++) {
        letter = getChar(appout);
        userOut += letter;
    }

    ignoreSpaces(appout);
    getNewLineEOF(appout);
    if (sans == "NIE" && userOut != "NIE") throw WrongAnswer("Wypisano slowo, a oczekiwano NIE.");
    if (sans != "NIE") {
        if (userOut <= ss1) throw WrongAnswer("To slowo jest mniejsze lub rowne niz pierwsze");
        if (userOut >= ss2) throw WrongAnswer("To slowo jest wieksze lub rowne niz drugie");
    }
}
