...
void DFS(int w) {   
    ...
    for (int i=0; i<G[w].size(); i++) {
        ...
        if (!visited[s.ff]) {
            ...
            neighSubtreeSum += subtree[s.ff];
            
            for (int j=1; j<=subtree[s.ff]; j++) {
                // assuming there is a bird in vertex 'w'
                for (int used=neighSubtreeSum-j; used>=1; used--) {
                ...
                for (int used=neighSubtreeSum-j-1; used>=0; used--) {
...
bool myComp(pair <int, int> a, pair <int, int> b) {
    return subtree[a.ff] < subtree[b.ff];
}
