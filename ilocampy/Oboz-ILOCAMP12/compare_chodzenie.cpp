...
const long double W = 0.0001L;
void spr(FILE *testin, FILE *appout, FILE *testout)
{
    long double prawidlowy;
    fscanf(testout, "%Lf", &prawidlowy);

    long double sprawdzany;
    fscanf(appout, "%Lf", &sprawdzany);

    if((sprawdzany-prawidlowy>0?sprawdzany-prawidlowy:prawidlowy-sprawdzany) > W)
    {
        stringstream ss;
        ss << "Za duża różnica: " << setprecision(10) << fixed << sprawdzany-prawidlowy << "!";
