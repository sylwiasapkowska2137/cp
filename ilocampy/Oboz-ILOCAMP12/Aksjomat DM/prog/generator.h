...
public:
    ...
    void addTest(const Test &test, int points);
    void generate() const;
...
private:
    ...
    // {test, points}
    std::vector<std::vector<std::pair<Test, int>>> tests;
...
inline void Generator::addTest(const Test &test, int points) {
    if (!insideGroup) {
        tests.emplace_back();
    }
    tests.back().push_back({test, points});
}
...
inline void Generator::generate() const {
    ...
    for (std::size_t groupIdx = 0; groupIdx < tests.size(); groupIdx++) {
        for (std::size_t testIdx = 0; testIdx < tests[groupIdx].size(); testIdx++) {
            ...
            std::cout << filename;
            createTestFile(inFilename, tests[groupIdx][testIdx].first);
...
inline std::string Generator::getFilename(std::size_t groupIdx, std::size_t testIdx) const {
    ...
    std::string group = std::to_string(groupIdx);
    
    if (tests.size() > 10 && groupIdx < 10 && groupIdx != 0) {
        group = "0" + group;
    }
    filename += group;
    ...
    if (tests[groupIdx][testIdx].second > 0) {
        filename += "-" + group + "p" + std::to_string(tests[groupIdx][testIdx].second);
    }
