...
{
    fscanf(testin, "%d%d", &n, &k);
    if(!(1 <= n && n < N && k <= N)) throw WrongAnswer("BLEDNY TEST1!");
    for(int i = 1;i <= n;i++)
        fscanf(testin, "%d", &A[i]);
    ...
    else
    {
        for(int i = 1;i <= n;i++)
        {
            B[i] = getLL(appout, 1, n);
            if(odw[B[i]]) throw WrongAnswer("Powtarza się liczba na wyjsciu!");
            odw[B[i]] = 1;
        ...
        if(perm_zloz(vector<int>(B, B+n+1), k) != vector<int>(A, A+n+1)) throw WrongAnswer("Po zlozeniu wychodzi co innego!");
    }
}
