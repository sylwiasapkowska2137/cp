...
void spr(FILE *testin, FILE *appout, FILE *testout) {
    ...
    for(int i = 1; i<=n; i++) {
        fscanf(testin, "%lld", &t[i]);
        t[i+n] = t[i];
        if(!(-MAXSUM <= t[i] && t[i] <= MAXSUM)) throw WrongAnswer("Błędny !");
    }

    for(int i = 1; i<=n; i++) {
        ans[i] = getLL(appout, 0, 0);
        
        if (ans[i] < -MAXSUM || ans[i] > MAXSUM) throw WrongAnswer("Niepoprawna odpowiedź (liczby spoza zakresu 10^9)"); 
    ...
    if (sum != t[1]) throw WrongAnswer("Niepoprawna odpowiedź.");
    
    for (int i=k+1; i<=2*n; i++) {
        sum -= ans[i-k];
        sum += ans[i];
        if (sum != t[i-k+1]) throw WrongAnswer("Niepoprawna odpowiedź.");
    }
}
